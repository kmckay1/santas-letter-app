...
// Shared recipient shape and print HTML for physical letters.
// Extracted from lib/lob.ts so the Lob and Stannp senders render identical
// letters from one source instead of drifting copies.
...
export function buildLetterHtml(child: ChildInfo, letterText: string, toAddress: MailAddress): string {
  ...
  const css = [
    ...
    // Outer gold border
    '.outer-border { position: absolute; inset: 0; border: 8px solid #c8922a; pointer-events: none; z-index: 1; }',

    // Corner diamonds
    '.corner { position: absolute; width: 16px; height: 16px; background: #c8922a; transform: rotate(45deg); z-index: 1; }',
    ...
    // Inner content
    '.p2-inner { margin: 10px; border: 1px solid rgba(200,146,42,0.35); padding: 20px 36px 90px; height: calc(11in - 189px); position: relative; }',
    ...
    // Footer
    '.p2-footer { position: absolute; bottom: 0; left: 0; right: 0; background: #6B0F0F; padding: 8px; text-align: center; }',
